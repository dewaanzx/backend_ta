...
const store = async (req, res) => {
  try {
    ...
    if (req.body.material_id) {
      await decreaseMaterialQuantity(req.body.material_id, req.body.quantity);
    }
        
    const today = new Date();
    const date = today.toISOString().split('T')[0];

    const today_transaction = await DailyTransaction.query().where('date', date).first();

    if (!today_transaction) {
      await DailyTransaction.query().insert({
        date: date,
        transaction: 1,
      });
    } else {
      await DailyTransaction.query().where('date', date).patch({
        transaction: parseInt(today_transaction.transaction) + 1,
      });
    }
